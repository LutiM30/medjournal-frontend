...
const Overview = () => {
    ...
    const fetchUserProfile = async () => {
      if (user?.uid) {
        ...
        if (profileData) {
          ...
          setValue('hospitalAffiliations', profileData.hospitalAffiliations?.join(', '));
          ...
          setValue('ageGroupsTreated', profileData.ageGroupsTreated?.join(', '));
          setValue('specificPatientGroups', profileData.specificPatientGroups?.join(', '));
          setValue('technologiesProficient', profileData.technologiesProficient?.join(', '));
        }
      }
    };
    fetchUserProfile();
  ...
  const onSubmit = async (data) => {
    const profileData = {
      ...data,
      hospitalAffiliations: data.hospitalAffiliations.split(',').map(item => item.trim()),
      subspecialties: data.subspecialties.split(',').map(item => item.trim()),
      procedures: data.procedures.split(',').map(item => item.trim()),
      ageGroupsTreated: data.ageGroupsTreated.split(',').map(item => item.trim()),
      specificPatientGroups: data.specificPatientGroups.split(',').map(item => item.trim()),
      technologiesProficient: data.technologiesProficient.split(',').map(item => item.trim()),
  ...
  return (
    <div className='max-w-4xl mx-auto p-6 mb-6 shadow-lg rounded-lg bg-white dark:bg-slate-900 border-l-4 border-blue-500'>
      <h2 className='text-2xl font-semibold mb-4 text-blue-600'>Doctor Profile</h2>
      <hr className='my-4' />
      ...
      {isProfileComplete && !isEditing ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-4'>
            <div>
              <label className='block text-left font-semibold text-purple-600'>Years of Experience</label>
              <input
                ...
                type='number'
              />
              {errors.yearsOfExperience && <ErrorMessage msg="This field is required." />}
            </div>
            <div>
              <label className='block text-left font-semibold text-purple-600'>Current Role</label>
              <input
                className='w-full p-2 border rounded'
                {...register('currentRole', { required: true })}
                placeholder='Current Role'
              />
              {errors.currentRole && <ErrorMessage msg="This field is required." />}
            </div>
            <div>
              <label className='block text-left font-semibold text-purple-600'>Hospital Affiliations</label>
              <input
                className='w-full p-2 border rounded'
                {...register('hospitalAffiliations', { required: true })}
                placeholder='Comma-separated list of hospital affiliations'
              />
              {errors.hospitalAffiliations && <ErrorMessage msg="This field is required." />}
            </div>
            <div>
              <label className='block text-left font-semibold text-purple-600'>Clinic Name</label>
              <input
                className='w-full p-2 border rounded'
                {...register('clinicName', { required: true })}
                placeholder='Clinic Name'
              />
              {errors.clinicName && <ErrorMessage msg="This field is required." />}
            </div>
          </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-4'>
            <div>
              <label className='block text-left font-semibold text-purple-600'>Primary Specialty</label>
              <input
                className='w-full p-2 border rounded'
                {...register('primarySpecialty', { required: true })}
                placeholder='Primary Specialty'
              />
              {errors.primarySpecialty && <ErrorMessage msg="This field is required." />}
            </div>
            <div>
              <label className='block text-left font-semibold text-purple-600'>Subspecialties</label>
              <input
                className='w-full p-2 border rounded'
                {...register('subspecialties', { required: true })}
                placeholder='Comma-separated list of subspecialties'
              />
              {errors.subspecialties && <ErrorMessage msg="This field is required." />}
            </div>
          </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-4'>
            <div>
              <label className='block text-left font-semibold text-purple-600'>Procedures Offered</label>
              <input
                className='w-full p-2 border rounded'
                {...register('procedures', { required: true })}
                placeholder='Comma-separated list of procedures'
              />
              {errors.procedures && <ErrorMessage msg="This field is required." />}
            </div>
            <div>
              <label className='block text-left font-semibold text-purple-600'>Age Groups Treated</label>
              <input
                className='w-full p-2 border rounded'
                {...register('ageGroupsTreated', { required: true })}
                placeholder='Comma-separated list of age groups treated'
              />
              {errors.ageGroupsTreated && <ErrorMessage msg="This field is required." />}
            </div>
          </div>

          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div>
              <label className='block text-left font-semibold text-purple-600'>Specialized Patient Groups</label>
              <input
                className='w-full p-2 border rounded'
                {...register('specificPatientGroups', { required: true })}
                placeholder='Comma-separated list of patient groups'
              />
              {errors.specificPatientGroups && <ErrorMessage msg="This field is required." />}
            </div>
            <div>
              <label className='block text-left font-semibold text-purple-600'>Medical Technologies/Techniques</label>
              <input
                className='w-full p-2 border rounded'
                {...register('technologiesProficient', { required: true })}
                placeholder='Comma-separated list of technologies'
              />
              {errors.technologiesProficient && <ErrorMessage msg="This field is required." />}
            </div>
          </div>
